...
Original file is located at
    https://colab.research.google.com/drive/1Z1Q9jLb2nFJKh97lS50BiRzXp4nv09kj
"""
...
class AntAlgorithm:
    def __init__(self, distances, start, n_ants, n_best, n_iterations, decay, alpha=1, beta=1):
        # Ініціалізація параметрів алгоритму мурахи
    ...
    def spread_pheromone(self, all_paths, n_best, shortest_path):
        # Розповсюдження феромону на кращі шляхи
        sorted_paths = sorted(all_paths, key=lambda x: x[1])
        for path, dist in sorted_paths[:n_best]:
            self.pheromone[path[:, 0], path[:, 1]] += 1.0 / dist

    def get_started(self):
        # Запуск алгоритму мурахи
        shortest_path = None
        all_time_shortest_path = ("placeholder", np.inf)
        for i in range(1, self.n_iterations + 1):
            all_paths = self.get_routes()
            ...
            if i % 50 == 0 or i == self.n_iterations:
                print("Крок {}: {}".format(i, shortest_path[0]))
                print("Загальна відстань: {} км".format(all_time_shortest_path[1]))
    ...
    def get_routes(self):
        # Отримання всіх можливих шляхів для мурах
        all_paths = []
        for _ in range(self.n_ants):
            path = self.get_path(self.current_position)
            all_paths.append((path, self.get_whole_distance(path)))
        return all_paths

    def get_path(self, start):
        # Отримання конкретного шляху для мурахи
        path = []
        tabu = set()
        tabu.add(start)
        previous_step = start
        for _ in range(len(self.distances) - 1):
            move = self.get_next_position(previous_step, tabu)
    ...
    def get_whole_distance(self, path):
        # Отримання загальної відстані для конкретного шляху
        return np.sum(self.distances[path[:, 0], path[:, 1]])

    def get_next_position(self, current_position, tabu):
        # Отримання наступної позиції для мурахи
        pheromone = np.copy(self.pheromone[current_position])
        pheromone[list(tabu)] = 0
        row = pheromone ** self.alpha * ((1.0 / self.distances[current_position]) ** self.beta)
        norm_row = row / row.sum()
        move = np_choice(self.all_inds, 1, p=norm_row)[0]
        return move

# Встановлення параметрів і запуск алгоритму
decay = 0.95
start = 4 # Варіант №5
...
for result in iteration_results:
    ...
    fig, ax = plt.subplots(figsize=(16, max(6, 0.3 * num_cities)))
    ax.plot(iteration_indices, y_ticks, marker='x',markeredgecolor='orange', linestyle='-', color='green', markersize=8, linewidth=2, label=f'Iteration {iteration_number} ({iteration_distance:.2f} km)')
    ax.set_xticks(range(1, num_cities + 1))
    ax.set_xticklabels(table_cities, fontsize=10, rotation=80)   # Поворот тексту на 80 градусів
    ax.set_yticks(y_ticks)
    plt.title(f'Маршрут пройдений мікровояжером (Ітерація {iteration_number})', fontsize=16)
    plt.xlabel('Назви міст', fontsize=14)
    plt.ylabel('Номера міст', fontsize=14)
